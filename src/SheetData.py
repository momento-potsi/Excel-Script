...
class ExcelSheetData(object):
    ...
    def updateEntries(self, tableData: list[list[str]] = None, anchor: str = None):
        ...
        for entry in self.entryList:
            ...
            match entry.dataType:
                case SheetDataEnum.Cell:
                    formatWrite(self.workBook, entry.style, (entry.toCellStr())[0], entry.data[0])

                case SheetDataEnum.MergedCell:
                    self.workBook.active.merge_cell((entry.toCellStr())[0], (entry.toCellStr())[1])
                    formatWrite(self.workBook, entry.style, (entry.toCellStr())[0], entry.data[0])

                case SheetDataEnum.Row:
                    if (len(entry.data) == range(entry.cellEnd[1] - entry.cellStart[1])) or (len(entry.data) >= range(entry.cellEnd[1] - entry.cellStart[1])):
                        for x in range(entry.cellEnd[1] - entry.cellStart[1]): # subtract columns
                            formatWrite(self.workBook, entry.style, (entry.toCellStr())[0], entry.data[x - 1])
                            entry.cellStart += 1
                    elif len(entry.data) <= range(entry.cellEnd[1] - entry.cellStart[1]): # repeat last data for remaining cells
                        for x in range(entry.cellEnd[1] - entry.cellStart[1]): # subtract columns
                            if x > len(entry.data):
                                formatWrite(self.workBook, entry.style, (entry.toCellStr())[1], entry.data[x - 1])
                            else:
                                formatWrite(self.workBook, entry.style, (entry.toCellStr())[0], entry.data[x - 1])
                            entry.cellStart += 1
                ...
                case SheetDataEnum.Column: 
                    if (len(entry.data) == range(entry.cellEnd[0] - entry.cellStart[0])) or (len(entry.data) >= range(entry.cellEnd[0] - entry.cellStart[0])):
                        for x in range(entry.cellEnd[0] - entry.cellStart[0]): # subtract rows
                            formatWrite(self.workBook, entry.style, (entry.toCellStr())[0], entry.data[x - 1])
                            entry.cellStart += 1
                    elif len(entry.data) <= range(entry.cellEnd[0] - entry.cellStart[0]): # repeat last data for remaining cells
                ...
                case SheetDataEnum.Table:
                    ...
                    for i, listValue in enumerate(tableData, start = entry.cellStart[0]):
                ...
                case SheetDataEnum.Image:
                    ...
                    img.anchor = anchor # where you want image to be anchored/start from 'A1'
